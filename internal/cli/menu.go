...
// showMenuAndExecute loops: shows menu, runs selected tool, returns to menu.
// Only exits on q/ctrl+c.
func showMenuAndExecute() error {
	for {
		...
		fmt.Println()
		fmt.Print(menuDesc.Render(fmt.Sprintf("  Chọn [1-%d] hoặc q để thoát: ", len(menuEntries))))
		...
		_ = cmd.Run() // ignore error, always return to menu

		fmt.Println()
		fmt.Println(menuDesc.Render("  ← Quay về menu chính..."))
	}
}
